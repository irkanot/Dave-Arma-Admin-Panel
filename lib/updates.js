...
Updates.prototype.feedUrl = function () {
  const configured = this.config.updates && this.config.updates.feedUrl
  if (configured) return configured
  const baseUrl = this.config.steamAuth && this.config.steamAuth.baseUrl
  return baseUrl ? baseUrl.replace(/\/$/, '') + '/releases/latest.json' : ''
}
